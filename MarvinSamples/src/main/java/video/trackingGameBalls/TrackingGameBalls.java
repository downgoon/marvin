...
public class TrackingGameBalls extends JFrame implements Runnable {
	...
	private MarvinImage imageIn, imageOut, imageHat, imageBall;
	...
	private int maskPx, maskPy, maskWidth = 120, maskHeight = 60;

	private MarvinImagePlugin pluginColorPattern, text;
	...
	public TrackingGameBalls() {
		try {
			...
			imageHat = MarvinImageIO.loadImage("src/main/resources/hat.png");
	...
	private void combineImage(MarvinImage a_image, int x, int y) {
		...
		for (int iy = 0; iy < l_height; iy++) {
			for (int ix = 0; ix < l_width; ix++) {
				if (ix + x > 0 && ix + x < imageWidth && iy + y > 0 && iy + y < imageHeight) {
	...
	private void collisionBallMask(Ball a_ball) {
		if ((a_ball.px >= maskPx && a_ball.px <= maskPx + maskWidth
